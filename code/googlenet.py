...
def build_googlenet(input_shape=(224, 224, 3), num_classes=2, weights="imagenet"):
    inputs = layers.Input(shape=input_shape)
    base = InceptionV3(weights=weights, include_top=False, input_tensor=inputs)
    x = base.output
...
def main():
    ...
    parser.add_argument('--weights_out', default='googlenet_path_to_my_weights.weights.h5')
    parser.add_argument('--weights', default='')
    parser.add_argument('--prefix', default='googlenet')
    parser.add_argument('--metrics_csv', default='')
    parser.add_argument('--variant', default='googlenet')
    ...
    model = build_googlenet(weights=None if args.no_pretrained else "imagenet")
    if args.weights:
