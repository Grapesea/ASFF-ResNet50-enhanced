...
def build_xception(input_shape=(224, 224, 3), num_classes=2, weights="imagenet"):
    inputs = layers.Input(shape=input_shape)
    base = Xception(weights=weights, include_top=False, input_tensor=inputs)
    x = base.output
    x = layers.GlobalAveragePooling2D()(x)
    x = layers.Dense(2048, activation='relu')(x)
    outputs = layers.Dense(num_classes, activation='softmax')(x)
    return models.Model(inputs=inputs, outputs=outputs, name='Xception')
...
def main():
    ...
    parser.add_argument('--weights_out', default='xception_path_to_my_weights.weights.h5')
    parser.add_argument('--weights', default='')
    parser.add_argument('--prefix', default='xception')
    parser.add_argument('--metrics_csv', default='')
    parser.add_argument('--variant', default='xception')
    ...
    model = build_xception(weights=None if args.no_pretrained else "imagenet")
    if args.weights:
