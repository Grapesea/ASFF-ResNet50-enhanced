...
class AdaptiveFeatureFusion(layers.Layer):
    ...
    def build(self, input_shape):
        self.dense1 = layers.Dense(self.channels, activation='relu')
        self.dense2 = layers.Dense(input_shape[0][-1], activation='softmax')
        self.conv = layers.Conv2D(filters=input_shape[0][-1], kernel_size=1, padding='same')
        self.reshape = layers.Reshape((1, 1, input_shape[0][-1]))
        super().build(input_shape)

    def call(self, inputs):
        feature1, feature2 = inputs
        target_size = tf.shape(feature1)[1:3]
        feature2 = tf.image.resize(feature2, target_size, method='bilinear')
        feature2 = self.conv(feature2)
        x = self.concat([feature1, feature2])
        x = self.gap(x)
        x = self.dense1(x)
        weights = self.dense2(x)
        weights = self.reshape(weights)
        return feature1 * weights + feature2 * (1.0 - weights)
...
def build_asff_resnet50(input_shape=(224, 224, 3), num_classes=2):
    inputs = layers.Input(shape=input_shape)
    base = ResNet50(weights='imagenet', include_top=False, input_tensor=inputs)

    branch1 = base.get_layer('conv4_block6_out').output
    branch2 = base.get_layer('conv5_block3_out').output

    fusion = AdaptiveFeatureFusion(1024)([branch1, branch2])

    x = layers.GlobalAveragePooling2D()(fusion)
    x = layers.Dense(2048, activation='relu')(x)
    outputs = layers.Dense(num_classes, activation='softmax')(x)

    model = models.Model(inputs=inputs, outputs=outputs, name='ASFF_ResNet50')
    return model
...
def main():
    ...
    parser.add_argument('--weights_out', default='asff_resnet50_path_to_my_weights.weights.h5')
    parser.add_argument('--weights', default='')
    parser.add_argument('--prefix', default='asff_resnet50')
    parser.add_argument('--metrics_csv', default='')
    parser.add_argument('--variant', default='asff')
