...
def _fire_module(x, squeeze, expand, name):
    sq = layers.Conv2D(squeeze, (1, 1), activation='relu', padding='same', name=f'{name}_squeeze')(x)
    e1 = layers.Conv2D(expand, (1, 1), activation='relu', padding='same', name=f'{name}_expand1x1')(sq)
    e3 = layers.Conv2D(expand, (3, 3), activation='relu', padding='same', name=f'{name}_expand3x3')(sq)
    return layers.Concatenate(name=f'{name}_concat')([e1, e3])
...
def build_squeezenet(input_shape=(224, 224, 3), num_classes=2):
    ...
    return models.Model(inputs=inputs, outputs=outputs, name='SqueezeNet')
...
def main():
    ...
    parser.add_argument('--weights_out', default='squeezenet_path_to_my_weights.weights.h5')
    parser.add_argument('--weights', default='')
    parser.add_argument('--prefix', default='squeezenet')
    parser.add_argument('--metrics_csv', default='')
    parser.add_argument('--variant', default='squeezenet')
